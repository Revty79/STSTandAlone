import type { CharacterAttributeKey, CharacterAuthorizedItem } from "../../types/character";
import { getAttributeModifier } from "./characterRules";
...
function weaponUses(item: CharacterAuthorizedItem): WeaponUse[] {
  ...
  const explicitlyRanged = /bow|crossbow|firearm|pistol|rifle|cannon|ranged/i.test(
    item.weaponType ?? "",
  );
...
export function getCharacterWeaponDamageSummary(
  ...
  const uses = weaponUses(item).map((use) => {
    ...
    return {
      ...use,
      modifier,
      totalDamage: addModifier(item.damage, modifier),
    };
  });
  ...
  return {
    modifier: uses.map((use) => `${use.attribute} ${signedNumber(use.modifier)}`).join(" / "),
    totalDamage: uses.map((use) => `${use.label === "Melee" ? "M" : "R"} ${use.totalDamage}`).join(" / "),
  };
}
