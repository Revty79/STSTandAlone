...
export function availableRandomMagicSystems(
...
export function createCompletelyRandomAnswers(
  ...
  return {
    name: "",
    raceId: aggregate.allowedRaces.length > 0
      ? pick(aggregate.allowedRaces, random).id
      : null,
...
function generateAttributes(
  ...
  while (remaining > 0.000_001) {
    const available = CHARACTER_ATTRIBUTE_KEYS.filter((key) => {
      ...
      return attributes[key] + 0.000_001 < cap;
    });
...
function generateSkills(
  ...
  const childIds = new Set(aggregate.skillRelationships
    .filter(({ relationshipType }) => relationshipType.toLocaleLowerCase() === "parent")
    .map(({ skillId }) => skillId));
  ...
  function eligibleCandidates(): SkillCandidate[] {
    ...
    const roots = aggregate.skillCatalog.filter((skill) =>
      !childIds.has(skill.id)
      && (skill.tier === null || skill.tier === 1));
    for (const skill of roots) {
      ...
      if ((allocation?.points ?? 0) + 0.000_001 < maximum) {
    ...
    for (const parent of allocations) {
      ...
      for (const skill of childrenByParent.get(parent.skillId) ?? []) {
        ...
        if (!racial.granted
          && getEffectiveSkillPoints(parent.points, race, parent.skillId) + 0.000_001 < threshold) {
          continue;
        }
        if (!isSkillAllowedByCampaign(
          skill,
        ...
        if (requiresCastingLevel(skill, rootSkill)) {
          ...
          const accessLevel = magicSystem
            ...
            : null;
          if (!magicSystem
            || !canAccessSupernaturalSkillAtLevel(skill, rootSkill, accessLevel)) continue;
        }
        const maximum = getCreationPurchasedSkillMaximum(
          skill,
          aggregate.campaign.maxStartingSkill,
  ...
  function candidateWeight(candidate: SkillCandidate): number {
    ...
    let weight = attribute
      ? ROLE_ATTRIBUTE_WEIGHTS[focus][attribute]
      : 1.5;
...
function generateItems(
  ...
  const equipment = aggregate.authorizedItems.filter((item) =>
    item.catalogScope.toLocaleLowerCase() === "equipment"
    && item.credits !== null
    && item.credits >= 0
    && item.credits <= budget);
  ...
  const preferred = preferredGroup
    ? equipment.filter(({ equipmentGroup }) => equipmentGroup?.toLocaleLowerCase() === preferredGroup)
    : equipment;
  ...
  return chosen.map((item) => ({
    itemId: item.id,
    quantity: 1,
    unitCostCredits: item.credits ?? 0,
  }));
}

function sizeIdentity(size: RaceAggregate["race"]["size"], random: () => number) {
  const profiles = {
  ...
  const profile = size ? profiles[size] : profiles.Medium;
