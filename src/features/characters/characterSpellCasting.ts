...
import type {
  SpellCastingSystem,
  SpellDocument,
} from "../spell-construction/models/spell";
import {
  getCharacterMagicSystem,
  getCharacterManaProfilesForAggregate,
...
export function getAvailableSpellCastingContexts(
  ...
  const profiles = getCharacterManaProfilesForAggregate(aggregate);
  const allowedSystems: SpellCastingSystem[] = spell.tradition === "Psionics"
    ? ["Psyonics"]
...
export function resolveCharacterSpellCastingContext(
  ...
  if (allocationId !== undefined) {
    ...
    system = allocation ? rootSystemForAllocation(aggregate, allocation) ?? undefined : undefined;
  }
  if (!system && spell.frameworkSkillId) {
    const frameworkSystems = new Set(
      aggregate.skillAllocations
        .filter(({ skillId, points }) => skillId === spell.frameworkSkillId && points > 0)
        .map((allocation) => rootSystemForAllocation(aggregate, allocation))
        .filter((candidate): candidate is SpellCastingSystem => candidate !== null),
    );
