import type { CharacterQuintessencePurchaseType } from "../../types/character";
...
export function getQuintessenceCost(
  ...
  if (!Number.isInteger(quantity) || quantity <= 0) return Number.POSITIVE_INFINITY;
  if (purchaseType === "attribute") return quantity * ATTRIBUTE_QUINTESSENCE_COST;
  if (purchaseType === "fatePoints") return quantity * FATE_POINT_QUINTESSENCE_COST;
  return quantity;
}
