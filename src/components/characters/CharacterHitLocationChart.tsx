...
function poolClass(poolKey: CharacterHpPoolKey): string {
  return `character-hit-chart__pool--${poolKey.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}`;
}
    ...
    <div className="character-hit-chart">
      ...
      <div className="character-hit-chart__layout">
        <figure>
          ...
          >
            <title id="character-hit-chart-title">Humanoid hit-location body target</title>
            <desc id="character-hit-chart-description">
              Results zero through nine identify the Head, arms, upper and lower legs,
              Groin, Stomach, and Chest. Repeated regions share a single HP pool.
            </desc>
            ...
            {breakdown.locations.map((location) => {
              ...
              return (
                ...
                >
                  ...
                  <circle r="25" />
                  <text className="character-hit-chart__target-number" textAnchor="middle" y="5">
                    {location.result}
                  </text>
                </g>
              );
          ...
          <figcaption>Character right appears on the viewer&apos;s left. Numbers identify hit results; HP is assigned once to each complete body region.</figcaption>
        </figure>
