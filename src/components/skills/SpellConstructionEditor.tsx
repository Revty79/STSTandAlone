...
import { SpellContainerEditor } from "./spell/SpellContainerEditor";
import { SpellModifierEditor } from "./spell/SpellModifierEditor";
import { ProgressiveSpellEditor } from "./spell/ProgressiveSpellEditor";
import type { SpellFrameworkSkill } from "../../types/skill";
...
export function SpellConstructionEditor({
  ...
  const [frameworkState, setFrameworkState] = useState<"loading" | "ready" | "error">(
    "loading",
  );
    ...
    <div className="spell-builder">
      <section className="spell-builder__identity">
        <div className="spell-builder__section-heading">
          <div><p>SPELL EXTENSION</p><h4>Construction Identity</h4></div>
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                update({
                  ...
                  discipline: identity.field === "discipline" ? selected?.name ?? "" : "",
                  resonance: identity.field === "resonance" ? selected?.name ?? "" : "",
                });
              }}
            >
              <option value="">
            ...
            {frameworkState === "ready" && frameworkOptions.length === 0 && (
              <small className="spell-builder__field-note">
                No eligible {identity.label.toLocaleLowerCase()} Skills exist in this tree yet.
              </small>
            )}
        ...
        <label>
          ...
          <textarea rows={3} value={document.description} onChange={(event) => update({ description: event.target.value })} />
        </label>
        <div className="spell-builder__inline-fields">
          <label><span>Flavor Line</span><input value={document.flavorLine} onChange={(event) => update({ flavorLine: event.target.value })} /></label>
          <label><span>Construction Notes</span><input value={document.notes} onChange={(event) => update({ notes: event.target.value })} /></label>
        </div>
      </section>
      ...
      <section className="spell-builder__section">
        <div className="spell-builder__section-heading">
          <div><p>RECURSIVE MODEL</p><h4>Base Construction</h4></div>
          <button type="button" onClick={() => update({ containers: [...document.containers, createContainer()] })}>
            Add Root Container
          </button>
        </div>
        ...
        <SpellModifierEditor selections={document.modifiers} onChange={(modifiers) => update({ modifiers })} />
      </section>

      <section className="spell-builder__section">
        <div className="spell-builder__section-heading">
          <div><p>SKILLS WITH EXTRA STEPS</p><h4>Progressive Spell</h4></div>
          <label className="spell-builder__checkbox">
            <input type="checkbox" checked={progressiveEnabled} onChange={(event) => setProgressive(event.target.checked)} />
        ...
        {progressiveEnabled ? (
          ...
          <p className="spell-builder__empty">This spell does not use inherited Progressive tiers.</p>
        )}
      </section>

      <section className={`spell-builder__validation is-${activeValidation.status.toLowerCase()}`}>
        <div className="spell-builder__section-heading">
          <div><p>CURRENT RULE PROFILE</p><h4>Validation: {activeValidation.status}</h4></div>
      ...
      <details className="spell-builder__breakdown">
        ...
        <table>
          <thead><tr><th>Component</th><th>Category</th><th>Mana</th></tr></thead>
          <tbody>
            {baseCalculation.breakdown.map((line, index) => (
              <tr key={`${line.id}-${index}`}>
                <td style={{ paddingLeft: `${12 + line.depth * 12}px` }}>{line.label}</td>
                <td>{line.category}</td>
