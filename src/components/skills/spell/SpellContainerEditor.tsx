  ...
  return (
    <article className="spell-builder__selection">
      <div className="spell-builder__selection-heading">
        <div><strong>{rule.name}</strong><span>{rule.componentMastery} component mastery</span></div>
      ...
      {rule.cost.kind === "scalable" && (
        <label>
          ...
          <input
            ...
            onChange={(event) =>
              onChange({ ...effect, quantity: Math.max(1, Number(event.target.value) || 1) })
            }
          />
        </label>
      )}
...
function AddOnSelectionEditor({
  ...
  return (
    <article className="spell-builder__selection">
      <div className="spell-builder__selection-heading">
        <div><strong>{label}: {rule.name}</strong><span>{rule.componentMastery} component mastery</span></div>
...
export function SpellContainerEditor({
  ...
  function addEffect() {
    ...
    if (!selected) return;
    const effect: SpellContainer["effects"][number] = {
      id: createStableId("effect"),
      ruleId: selected.id,
      quantity: 1,
      description: "",
    };
    onChange({ ...container, effects: [...container.effects, effect] });
    setEffectRuleId("");
  }

  function addDuration() {
    const selected = serrianTideRules.durations.find(
      ({ id }) => id === durationRuleId,
    );
    if (!selected) return;
    onChange({
      ...container,
  ...
  return (
    <article className="spell-container" style={{ "--container-depth": depth } as React.CSSProperties}>
          ...
          <section className="spell-builder__subsection">
            <div className="spell-builder__subheading">
              ...
              <div className="spell-builder__add-row">
                <select value={effectRuleId} onChange={(event) => setEffectRuleId(event.target.value)}>
