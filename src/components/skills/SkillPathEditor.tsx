  ...
  const attributes = [context.primaryAttribute, context.secondaryAttribute]
    .filter(
      (attribute): attribute is string =>
        Boolean(attribute?.trim()) && attribute?.toLocaleUpperCase() !== "N/A",
    )
    .join(" / ");
    ...
    <div className="skill-path-editor">
      ...
      <div className="skill-path-editor__relationships">
        {relationships.length === 0 ? (
          <p className="skill-editor__empty">No parent or prerequisite relationships are attached.</p>
        ) : (
          relationships.map((relationship, index) => (
            ...
            >
              <div>
                <strong>{relationship.relatedSkillName ?? `Skill ${relationship.relatedSkillId}`}</strong>
                <label>
              ...
              <div className="skill-path-editor__order">
                ...
                <button
                  className="is-danger"
                  type="button"
                  onClick={() => onChange(relationships.filter((_, current) => current !== index))}
                >
                  Remove
                </button>
              </div>
