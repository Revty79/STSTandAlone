...
export function SpellCastingPanel({
  ...
  const [circumstance, setCircumstance] = useState<RawCastingCircumstanceId>();
  ...
  const progressiveResolution = useMemo(
    () => hasProgressiveSpellModifier(spell)
      ? resolveProgressiveSpellForLevel(spell, practitionerLevel ?? "Apprentice")
      : null,
    [practitionerLevel, spell],
  );
  const castingCalculation = progressiveResolution?.castingCalculation ?? baseCalculation;
  ...
  const finalCast = useMemo(() => {
    const effectiveCircumstance = automaticKnownSpell ? "have-spell" : circumstance;
    if (!effectiveCircumstance || (automaticKnownSpell && !practitioner)) return null;
    return practitioner
  ...
  return (
    <section className="spell-casting-panel">
      <div className="spell-player-section-heading">
        <div><p>CASTING CONTEXT</p><h3>{automaticKnownSpell ? "Known Spell Cost" : "Practitioner & Impromptu Cast"}</h3></div>
        <span>{automaticKnownSpell ? "Calculated from this Character." : "Construction values remain unchanged."}</span>
      </div>
      ...
      {progressiveResolution ? (
        <p className="spell-casting-panel__note">
          Active Progressive tier: <strong>{progressiveResolution.level}</strong>. Casting still uses the original base cost.
        </p>
      ) : null}
      {automaticKnownSpell ? (
        <p className={`spell-casting-panel__note${practitionerLevel ? "" : " is-error"}`}>
          {practitionerLevel
            ? <>Using <strong>{castingSystem}</strong> at <strong>{practitionerLevel}</strong> level with <strong>{manaPool ?? 0} Mana</strong>. Because this Spell is in the Character&apos;s Spellbook, <strong>I Have the Spell</strong> is automatic.</>
            : <>This Character does not currently have a usable caster level for this Spell&apos;s magic system, so its casting cost cannot be calculated.</>}
        </p>
      ) : (
        <div className="spell-casting-panel__controls">
          <fieldset>
            ...
            <div className="spell-casting-panel__choices">
              <button type="button" className={!practitionerLevel ? "is-active" : ""} onClick={() => onPractitionerLevelChange?.(undefined)}>Not Set</button>
              {PRACTITIONER_LEVELS.map((level) => (
                <button type="button" key={level} className={practitionerLevel === level ? "is-active" : ""} onClick={() => onPractitionerLevelChange?.(level)}>{level}</button>
              ))}
            </div>
          </fieldset>
          <fieldset>
      ...
      <div className="spell-casting-panel__results">
        <div>
          ...
          <strong>{automaticKnownSpell ? practitionerLevel ?? "N/A" : practitioner ? practitioner.adjustedManaCost : "N/A"}</strong>
          <small>{automaticKnownSpell
            ? practitionerLevel ? `${castingSystem} · ${manaPool ?? 0} Mana available` : "No eligible casting profile"
            : practitioner ? `${practitioner.combatCastingTime} Initiative · ${practitioner.outOfCombatCastingTimeSeconds}s` : "Choose a Practitioner Level"}</small>
        </div>
        <div>
          <span>{automaticKnownSpell ? "Known Spell Cost" : "Final Cast Mana"}</span>
          <strong>{finalCast ? finalCast.finalCastingMana : "N/A"}</strong>
          <small>{finalCast ? `${finalCast.finalCombatCastingTime} Initiative · ${finalCast.finalOutOfCombatCastingTimeSeconds}s` : automaticKnownSpell ? "Caster level is unavailable" : "Choose a Raw Casting circumstance"}</small>
        </div>
      </div>
    </section>
  );
